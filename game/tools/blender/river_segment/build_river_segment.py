...
NO_RENDER = False
CONTROL = False
# Crack seams exist in the mesh (3-4 cm grooves) but carry the same albedo as
# the faces, so they read as flat at play distance. A real crack collects
# shadow and damp sediment: its interior runs 40-60 % darker with a soft
# falloff. CRACK_DARK multiplies the vertex colour of the seam strip; the
# one-ring falloff uses its square root.
CRACK_DARK = 0.55
TAG = ""
i = 0
while i < len(argv):
...
# Ramp (Joan picks): WEAR -- what fraction of the convex edge network the
# river has eaten (v5, Joan on v4: "pasaste de un extremo a otro... las
# piedras de rios son irregulares, grietas, cantos, partes redondeadas, otras
# lisas, otras asperas"). Per-stone jitter so no two stones match.
RAMP = {1: 0.30, 2: 0.50, 3: 0.70}
WEAR = RAMP[VARIANT]
RELIEF = 0.12                        # kept only for the v4-egg CONTROL
...
def shape_sharp_frac(faces):
    """Fraction of interior edges with dihedral > 30 deg."""
    edges = {e for f in faces if f.is_valid for e in f.edges}
    ne = ns = 0
    for e in edges:
        ...
        if math.degrees(e.calc_face_angle(0.0)) > 30.0:
            ns += 1
    return ns / max(ne, 1), ne


def shape_flat_frac(faces, tol_deg=8.0, min_patch=0.02):
    """Area fraction in flat PATCHES: region-grow over adjacent faces near the
    seed's normal, then the PLANARITY check the definition actually states --
    every face within tol of the patch's own mean plane. Without it a smoothly
    CURVED cap 2x tol wide counted as flat (measured: h2 stuck at 0.46-0.53
    because worn shoulders and big flats both passed a seed-only test)."""
    faces = [f for f in faces if f.is_valid]
    fset = set(faces)
    total = sum(f.calc_area() for f in faces)
    seen = set()
    flat = 0.0
        ...
        while queue:
            ...
            for e in g.edges:
                for h in e.link_faces:
                    if h in fset and h not in seen and h.normal.dot(seed_n) >= cos_tol:
                        seen.add(h); patch.append(h); queue.append(h)
        area = sum(g.calc_area() for g in patch)
        if area < min_patch:
            continue
        mean = Vector((0, 0, 0))
        for g in patch:
            mean += g.normal * g.calc_area()
        if mean.length < 1e-9:
            continue
        mean.normalize()
        if all(g.normal.dot(mean) >= cos_tol for g in patch):
            flat += area
    return flat / max(total, 1e-9)


def shape_radius_cv(verts):
    """Coefficient of variation of centroid->vertex distance."""
    vs = [v.co for v in verts if v.is_valid]
    if len(vs) < 4:
        return 0.0
    c = Vector((0, 0, 0))
    for p in vs:
        c += p
    c /= len(vs)
    ds = [(p - c).length for p in vs]
    m = sum(ds) / len(ds)
    var = sum((d - m) ** 2 for d in ds) / len(ds)
    return math.sqrt(var) / max(m, 1e-9)
...
if CONTROL:
    ...
    # shape-gate controls: the v3 faceted block must FAIL on the sharp /
    # all-flats side; the v4 egg must FAIL on the round side. A window a
    # control cannot reach from either side is not a window.
    ...
    tb.normal_update()
    fa_h1, _ = shape_sharp_frac(tb.faces[:]); fa_h2 = shape_flat_frac(tb.faces[:])
    fa_h3 = shape_radius_cv(tb.verts[:]); tb.free()
    facet_fails = fa_h1 > 0.25 or fa_h2 > 0.40
    print(f"[control facet] h1 {fa_h1:.2f} h2 {fa_h2:.2f} h3 {fa_h3:.2f} -> {'FAIL (sharp side)' if facet_fails else 'PASS'} (must FAIL)")
    tb = bmesh.new()
    bmesh.ops.create_uvsphere(tb, u_segments=14, v_segments=9, radius=0.5)
    for v in tb.verts:
        p = v.co * 3.4
        n = fbm(np.array([[p.x, p.y, p.z]]), 2, 5.0)[0] - 0.5
        v.co += v.co.normalized() * ((n - 0.25) * RELIEF * 1.0)
    for v in tb.verts:
        v.co.z *= 0.6
    for _ in range(2):
    ...
    tb.normal_update()
    eg_h1, _ = shape_sharp_frac(tb.faces[:]); eg_h2 = shape_flat_frac(tb.faces[:])
    eg_h3 = shape_radius_cv(tb.verts[:]); tb.free()
    egg_fails = eg_h1 < 0.08 and eg_h2 < 0.15
    print(f"[control egg]   h1 {eg_h1:.2f} h2 {eg_h2:.2f} h3 {eg_h3:.2f} -> {'FAIL (round side)' if egg_fails else 'PASS'} (must FAIL)")
    if ok_e or cov >= 0.60 or ok_g or not facet_fails or not egg_fails:
        raise SystemExit("[control] a control went the wrong way: the gate cannot see the defect")
    print("[control] OK: e/f/g fail on v2's layout; the shape window rejects both the die and the egg")
    raise SystemExit(0)
...
def bake_roughness(size, seed):
    """Greyscale roughness: matte base with polished patches (a worn stone is
    lisa in parts, aspera in others -- Joan's v5 correction). Tiled world-space
    like the albedo, so each stone crosses several zones."""
    u, v = np.meshgrid(np.linspace(0, 1, size, endpoint=False),
                       np.linspace(0, 1, size, endpoint=False))
    tw = 2.0 * math.pi
    p = np.stack([np.cos(u * tw), np.sin(u * tw), np.cos(v * tw) + np.sin(v * tw) * 0.7], -1)
    zones = fbm(p * 2.2, 2, seed)
    g = value_noise(p * 13.0, seed + 3.0)
    r = 0.78 + 0.10 * (g - 0.5)
    polished = np.clip((zones - 0.60) / 0.10, 0, 1)
    r = r * (1.0 - polished) + (0.50 + 0.06 * (g - 0.5)) * polished
    return np.clip(np.stack([r, r, r], -1), 0, 1)
...
print(f"[river] v5 variant w{VARIANT}: wear {WEAR:.0%} of edges eaten, wall boulders {WALL_LO:.1f}-{WALL_HI:.1f} m, wet band {WET_BAND:.2f} m")
...
img_rough = save_image("river_limestone_rough", bake_roughness(TEX_SIZE, 71.0), False)
...
MAT_LIME = textured_material("river_limestone_dry", img_lime, img_lime_n, 0.80, rough_img=img_rough)
MAT_LIME_WET = textured_material("river_limestone_wet", img_lime_wet, img_lime_n, 0.50)
MAT_LIME_WET_LOW = textured_material("river_limestone_wet_low", img_lime_wet, img_lime_n, 0.28)
...
bm = bmesh.new()
vcol = {}
# crack seam membership as a vert int layer (2 = seam strip, 1 = one-ring),
# NOT a set of BMVert refs: later bmesh ops realloc verts and dead refs raise
# ReferenceError even on hashing (measured: all 3 crackdark ramp builds died)
lay_crack = bm.verts.layers.int.new("crackdark")
...
def place_worn(kind, center_xy, radius, yaw, tilt_axis, tilt_deg, scale3v,
               burial, wear=None, z_top=None, z_rest=None, side=0.0,
               hide_z=None, cuts=7, bedding=1, elongate=1.25, flatten=0.85,
               subdiv_rounds=2, subth=0.15, jitter=0.08):
    """Drop logic shared with v3/v4, body = a worn block. `wear` defaults to
    the ramp value with per-stone jitter: no two stones match."""
    w = wear if wear is not None else max(0.05, min(0.95, WEAR + rng.uniform(-0.08, 0.08)))
    tb = make_worn_block(radius, w, rng, cuts=cuts, bedding=bedding,
                         elongate=elongate, flatten=flatten,
                         subdiv_rounds=subdiv_rounds, subth=subth, jitter=jitter)
    lay_src = tb.verts.layers.int.get("crackdark")
    M = (Matrix.Rotation(math.radians(yaw), 4, 'Z')
         @ Matrix.Rotation(math.radians(tilt_deg), 4, tilt_axis)
         @ Matrix.Diagonal((scale3v[0], scale3v[1], scale3v[2], 1.0)))
    vmap = {}
    for v in tb.verts:
        vmap[v] = bm.verts.new(M @ v.co)
    faces = []
    for f in tb.faces:
        try:
        ...
        nf.smooth = f.smooth
        faces.append(nf)
    for v, gv in vmap.items():
        if v[lay_src]:
            gv[lay_crack] = v[lay_src]
    tb.free()
    verts = set(vmap.values())
    ...
    if z_rest is not None:
        ...
        dz = max(min(dz, dz_ring), (WATER_Z - 0.06) - zmax)
    else:
    ...
    rec = dict(kind=kind, faces=faces, verts=verts, family="worn", height=h,
...
for (ti, side) in wall_tramos:
    ...
    lower = []
    x = x0 + 0.55
    while x < x1 - 0.35 and len(lower) < 6:
        ...
        rec = place_worn("wall", (x, meander(x) + side * d), a,
                         rng.uniform(0, 360), 'X', -side * rng.uniform(0, 12),
                         (1.0, b / a, rng.uniform(1.25, 1.55)),
                         burial=rng.uniform(0.25, 0.35), hide_z=WATER_Z, side=side,
                         cuts=7, bedding=rng.choice([1, 2]), flatten=1.0, subth=0.18, jitter=0.10)
        lower.append(rec)
        x = rec["xmax"]
    uppers = []
    for k in range(len(lower) - 1):
        ...
        rec = place_worn("wall", (xj, yj), a,
                         rng.uniform(0, 360), 'X', -side * rng.uniform(0, 10),
                         (1.0, b / a, rng.uniform(1.15, 1.45)),
    ...
    # crest: recompute the best x-adjacent pair EVERY iteration, so a fresh
    # crest immediately becomes a support candidate (a frozen pair list made
    # a third course impossible and short worn stones never cleared the bank)
    attempts = 0
    used_pairs = set()
    while attempts < 8 and ((_n_tall() < 3 and len(lower) + len(uppers) < 14) or len(lower) + len(uppers) < 11):
        if _n_tall() >= 3:
            break
        attempts += 1
        allr = sorted(lower + uppers, key=lambda r: r["center"][0])
        best = None
        ...
        a = 0.62 * S; b = a * rng.uniform(0.75, 0.95)
        xj = 0.5 * (A["xmax"] + B["xmin"]) + rng.uniform(-0.06, 0.06)
        xj = max(x0 + 0.35, min(x1 - 0.35, xj))
        ...
        rec["support"] = [A, B]
        uppers.append(rec)
    print(f"[wall] tramo {ti} side {side:+.0f}: {len(lower)}+{len(uppers)} stones, {_n_tall()} above the bank")
    ...
    while n < 8:                                          # fill the toe, on the ground
        ...
        place_worn("wall", (gx, meander(gx) + side * d), a,
                   rng.uniform(0, 360), 'Y', rng.uniform(-10, 10),
                   (1.0, b / a, rng.uniform(1.0, 1.3)),
                   burial=rng.uniform(0.30, 0.40), hide_z=WATER_Z, side=side,
                   cuts=6, bedding=1, flatten=1.0, subth=0.18, jitter=0.10)
        n += 1
...
for (ti, side) in slab_tramos:
    ...
    for k in range(n):
        ...
        y = meander(x) + side * d
        place_worn("slab", (x, y), a,
                   90.0 + rng.uniform(-30.0, 30.0), rng.choice(['X', 'Y']),
                   rng.uniform(-6.0, 6.0), (1.0, rng.uniform(0.7, 0.95), rng.uniform(0.55, 0.70)),
                   burial=0.0, z_top=WATER_Z + rng.uniform(-0.04, 0.04), side=side,
                   hide_z=WATER_Z,   # a slab base on the shelf sits under the waterline
                   cuts=6, bedding=2, flatten=0.9, subth=0.22)

# 3) BOULDER scale -- two worn groups in the channel.
GROUPS = [(-3.4, -0.6, 3), (2.6, 0.9, 2)]
for gi, (gx, gy, n) in enumerate(GROUPS):
    for k in range(n):
        ...
        place_worn("boulder", (cx, cy), a,
                   rng.uniform(0, 360), rng.choice(['X', 'Y']), rng.uniform(-20, 20),
                   (rng.uniform(1.0, 1.3), 1.0, rng.uniform(0.65, 0.9)),
                   burial=BURIAL, cuts=7, bedding=1, subth=0.22)
...
while len(bumps) < 650 and n_try < 8000:
    ...
    # worn-block hump: flat tilted top + rounded shoulder (a dome is a sphere)
    tx = rng.uniform(-0.30, 0.30); ty = rng.uniform(-0.30, 0.30)
    bumps.append((x, y, rx, ry, hb, yaw, tx, ty))
...
BSTEP = 0.135
_bucket = {}
for _bi, _b in enumerate(bumps):
    _bucket.setdefault((int(_b[0] // 0.3), int(_b[1] // 0.3)), []).append(_bi)
    ...
    for ox in (-1, 0, 1):
        for oy in (-1, 0, 1):
            for bi in _bucket.get((int(x // 0.3) + ox, int(y // 0.3) + oy), ()):
                bx, by, rx, ry, hb, yaw, tx, ty = bumps[bi]
                dx = x - bx; dy = y - by
                u = (dx * math.cos(-yaw) - dy * math.sin(-yaw)) / rx
                v = (dx * math.sin(-yaw) + dy * math.cos(-yaw)) / ry
                q = u * u + v * v
                if q < 1.0:
                    # flat top with a per-stone tilt, shoulder falls as smoothstep
                    top = 1.0 + tx * u + ty * v
                    hp = hb * top * smoothstep(0.0, 0.55, 1.0 - math.sqrt(q))
                    h = max(h, hp)
    return h
...
def in_strip(x, y):
    if bump_h(x, y) > 0.0:
        return True
    for ox in (-0.06, 0.0, 0.06):
        for oy in (-0.06, 0.0, 0.06):
            if bump_h(x + ox, y + oy) > 0.0:
                return True
    return False
...
for k in range(4):
    ...
    place_worn("cobble", (x, meander(x) + side * d), a,
               rng.uniform(-15, 15), 'Y', -rng.uniform(10, 20),
               (rng.uniform(1.1, 1.35), 1.0, rng.uniform(0.5, 0.7)),
               burial=rng.uniform(0.20, 0.40),
               wear=max(0.1, min(0.95, WEAR + 0.15 + rng.uniform(-0.08, 0.08))),
               cuts=5, bedding=0, subdiv_rounds=1, subth=0.20)
...
# --------------- metrics (h1/h2/h3): windowed shape gate. LECCIONES 8 note:
# a one-sided threshold pushes to the opposite extreme -- v4's "roundness <=
# 0.10" manufactured eggs. Every part has a WINDOW; two controls bracket it.
bm.normal_update()
wet_faces = []; wet_recs = []
for r in rocks:
    if not r.get("rounded"):
        continue                                   # dry faceted ledges exempt
    if r["bottom"] > WATER_Z + WET_BAND:
        continue
    wet_recs.append(r)
    # "in wet zone" scopes the FACES, not whole stones: the dry top half of a
    # tall wall boulder is where flats legitimately survive (the river does
    # not work it) -- counting it drowned the aggregate in dry flats
    wet_faces.extend(f for f in r["faces"]
                     if f.is_valid and f.calc_center_median().z < WATER_Z + WET_BAND)
h1, _ne = shape_sharp_frac(wet_faces)
h2 = shape_flat_frac(wet_faces)
# diagnostic: who carries the flats (area share x flat frac per kind)
_tot_area = sum(f.calc_area() for f in wet_faces)
for _k in ("wall", "slab", "boulder", "cobble"):
    _kf = []
    for r in wet_recs:
        if r["kind"] != _k:
            continue
        _kf.extend(f for f in r["faces"]
                   if f.is_valid and f.calc_center_median().z < WATER_Z + WET_BAND)
    if _kf:
        _ka = sum(f.calc_area() for f in _kf)
        print(f"[metric h2:{_k}] flat {shape_flat_frac(_kf):.3f} over {_ka:.2f} m2 ({_ka / max(_tot_area, 1e-9):.0%} of wet area)")
_cvs = [(shape_radius_cv(r["verts"]), r["kind"]) for r in wet_recs]
h3_min, h3_kind = min(_cvs, key=lambda t: t[0])
print(f"[metric h1] wet-zone sharp-edge frac {h1:.3f} (window 0.08-0.25, {_ne} edges, {len(wet_recs)} stones)")
print(f"[metric h2] flat-patch area frac {h2:.3f} (window 0.15-0.40)")
print(f"[metric h3] min per-stone radius CV {h3_min:.3f} on a {h3_kind} (limit >= 0.12)")
if not (0.08 <= h1 <= 0.25):
    raise SystemExit("[metric h1] FAIL: outside the worn-dice window")
if not (0.15 <= h2 <= 0.40):
    raise SystemExit("[metric h2] FAIL: outside the worn-dice window")
if h3_min < 0.12:
    raise SystemExit("[metric h3] FAIL: a stone is too regular")
...
for zc in (WATER_Z + WET_BAND, WATER_Z + 0.10, WATER_Z - 0.06):
...
# (matte), 4 bank soil, 5 limestone wet-low (the only glossy slot: the lowest
# 10 cm at the waterline -- "no 0.25 anywhere" else)
SLOT_GRAVEL, SLOT_GRAVEL_WET, SLOT_LIME, SLOT_LIME_WET, SLOT_BANK, SLOT_LIME_WET_LOW = range(6)
for f in bm.faces:
    ...
    if f in bed_face_set:
        ...
        if zc < WATER_Z + 0.10:
            f.material_index = SLOT_LIME_WET_LOW
        elif zc < WATER_Z + WET_BAND:
            f.material_index = SLOT_LIME_WET
        else:
            f.material_index = SLOT_LIME
...
# crack seam darkening: applied AFTER AO so the groove keeps its occlusion and
# adds sediment tone on top. Seam strip x CRACK_DARK, one-ring x sqrt (falloff).
_ring_f = CRACK_DARK ** 0.5
_nd = _nr = 0
for v in bm.verts:
    mark = v[lay_crack]
    if mark == 0 or v not in vcol:
        continue
    if mark == 2:
        vcol[v] = tuple(c * CRACK_DARK for c in vcol[v]); _nd += 1
    else:
        vcol[v] = tuple(c * _ring_f for c in vcol[v]); _nr += 1
print(f"[crackdark] seam {_nd} verts x{CRACK_DARK:.2f}, ring {_nr} x{_ring_f:.2f}")
if _nd == 0:
    raise SystemExit("[crackdark] FAIL: no seam verts reached the global mesh")

# --------------------------------------------- metric (b): buried bottom ring
hidden = 0; checked = 0; _bfail = {}
for r in rocks:
    if r["supported"]:
    ...
    if frac >= 0.85:
        ...
        _bfail[r["kind"]] = _bfail.get(r["kind"], 0) + 1
frac_hidden = hidden / checked
if _bfail:
    print(f"[metric b] visible bases by kind: {_bfail}")
...
fps = []
for (bx, by, brx, bry, bhb, byaw, _tx, _ty) in bumps:
...
ALL_MATS = (MAT_GRAVEL, MAT_GRAVEL_WET, MAT_LIME, MAT_LIME_WET, MAT_BANK, MAT_LIME_WET_LOW)
...
ob_bed = finalize(bm, True, f"river_bed_v5_w{VARIANT}{TAG}")
ob_rocks = finalize(bm, False, f"river_rocks_v5_w{VARIANT}{TAG}")
bm.free()
...
# ------------------------------------------------------------------- export
glb_path = os.path.join(OUT_GLB_DIR, f"river_segment_v5_w{VARIANT}{TAG}.glb")
...
with open(glb_path, "rb") as _fh:
    _fh.seek(12)
    _ln, _ty = _struct.unpack("<I4s", _fh.read(8))
    _js = _json.loads(_fh.read(_ln))
_mr = {m.get("name"): m.get("pbrMetallicRoughness", {}) for m in _js.get("materials", [])}
if "metallicRoughnessTexture" in _mr.get("river_limestone_dry", {}):
    print("[roughness] CONFIRMED: river_limestone_dry ships a roughness texture in the GLB")
else:
    print("[roughness] MISSING in GLB: exporter dropped the roughness texture -- dry stone ships constant 0.80 (trade, report it)")
...
def shoot(name, loc, target, lens):
    ...
    path = os.path.join(RENDER_DIR, f"river_v5_w{VARIANT}{TAG}_{name}.png")
