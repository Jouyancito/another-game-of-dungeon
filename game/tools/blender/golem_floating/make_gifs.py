...
# renders/golem_<clip>.gif (512x640, matches build_golem.py's anim frame
# render resolution). Run: python make_gifs.py
import glob
import os
...
FPS = 24
FRAME_STEP = 2  # build_golem.py renders every 2nd frame
...
for clip in CLIPS:
    ...
    out_path = os.path.join(REN_DIR, f"golem_{clip}.gif")
    frames[0].save(
        out_path,
        save_all=True,
